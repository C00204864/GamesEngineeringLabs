...
/// </summary>
Game::Game() : m_player(), m_healthSystem(), m_controlSystem(), m_aiSystem(), m_renderSystem()
{
	// Initialise SDL
	...
	SDL_SetRenderDrawColor(p_renderer, 0, 0, 0, 255); // Black Opaque Background

	// Initialise Game Variables

	// Extra info for systems
	m_renderSystem.init(p_renderer);
	m_aiSystem.init(1080, 720);

	// Player
	m_player.addComponent(new HealthComponent(100));
	m_player.addComponent(new PositionComponent(100, 100));
	m_player.addComponent(new RenderComponent("img\\Player.png", 100, 100, p_renderer));

	// Alien
	m_alien.addComponent(new HealthComponent(150));
	m_alien.addComponent(new PositionComponent(200, 200));
	m_alien.addComponent(new RenderComponent("img\\Alien1.png", 100, 100, p_renderer));

	// Dog
	m_dog.addComponent(new HealthComponent(75));
	m_dog.addComponent(new PositionComponent(300, 300));
	m_dog.addComponent(new RenderComponent("img\\Alien2.png", 100, 100, p_renderer));

	// Cat
	m_cat.addComponent(new HealthComponent(50));
	m_cat.addComponent(new PositionComponent(400, 400));
	m_cat.addComponent(new RenderComponent("img\\Alien3.png", 100, 100, p_renderer));
